class DrumKit {
	constructor() {
		...
		this.currentKcick = './sounds/kick-classic.wav';
		this.currentSnare = './sounds/snare-acoustic01.wav';
		this.currentHihat = './sounds/hihat-acoustic01.wav';
		this.controls = document.querySelectorAll('.controls');
		this.pads = document.querySelectorAll('.pad');
		this.trackBeats = this.pads.length / this.controls.length;
	...
	repeat(){
		let step = this.index % this.trackBeats;
		const activeBars = document.querySelectorAll(`.b${step}`);
		// Loop over the pads
		activeBars.forEach(bar => {
			bar.style.animation = `playTrack 0.3s alternate 2`;
